...
async function fixPages(ws, page, links, logger) {
    if (!fs.existsSync(path.join(links.save_path))) {
        createDirSync(path.join(links.save_path));
    }
    if (links.type === 1) {
    ...
    if (!/(https?|ftp|file):\/\/[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]/.test(links.url)) {
        let response = {
            id: links.id,
    ...
    await page.evaluate(() => {
        ...
        if (loginDialog) {
            let isTaobao = loginDialog.querySelector('iframe') && loginDialog.querySelector('iframe').src.search('login.taobao.com') !== -1;
            if (isTaobao) {
    ...
    let shot = new Promise(async (res) => {
        const sPath = `${links.save_path}/${links.id}.png`;
        const sType = 'png';
